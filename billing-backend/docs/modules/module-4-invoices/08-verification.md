...
| `scripts/verify-pdf.sh` | `npm run verify:pdf` | 29 | PDF rendering (Task 4.5) plus the Proforma split, B2C GSTIN-hiding, and visual-parity fixes (Task 4.7): the Yellow/Blue tax templates and the split Proforma Local/Outstation templates plus the credit-note template generate real, correctly-typed PDFs; DRAFT invoices reject generation; regeneration is idempotent; cross-tenant access is denied; Task 4.7 adds text-content assertions (column headers, the Proforma disclaimer, GSTIN presence/absence) that Task 4.5's original 18 checks deliberately didn't attempt |
...
**`verify:pdf`** — proves PDF GENERATION happened correctly (file exists, correct `Content-Type`, `%PDF` magic bytes, byte length matches the stored `pdf_file_size_bytes`, cross-tenant access denied) — Task 4.5's original 18 checks deliberately did NOT assert exact rendered byte content, since that's a Chromium-version concern rather than application logic (Rule 11). This is exactly why the module's real content bug (ADR-015's state-code lookup, see `known-issues.md`) passed every one of those 18 checks despite "Place of Supply" never actually rendering. Task 4.7 adds 11 checks that DO assert specific text content via `pdftotext` (a column header is present, a disclaimer paragraph is present, a GSTIN row is absent) — a deliberately narrower exception to Rule 11's general byte-content exemption, because the whole point of those checks is a specific string's presence/absence, which byte-length alone can never catch. Per Rule 13, these 11 checks are still necessary but not sufficient — the actual acceptance gate for Task 4.7 was a visual comparison against `fixtures/reference-pdfs/PTT-150/151/152.pdf`, which caught one real bug (`Place of Supply` leaking onto the Proforma header via Handlebars context inheritance) that no text-content assertion had been written to catch.

## Baseline

As of Task 4.7 completion: 19 verify scripts (395 checks — see the project's own `package.json` `scripts` section for the exact, currently-registered set) plus 27 pure unit tests (`test:gst` + `test:pricing`), 422 checks total, all green, confirmed by a full regression run immediately before this documentation pass was finalized. This document's script table is kept in sync with `package.json` by hand and should be cross-checked against it if the two ever appear to disagree.
